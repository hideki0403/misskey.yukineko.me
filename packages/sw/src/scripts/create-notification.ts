		...
		*/
		case 'notification':
			switch (data.body.type) {
				...
				case 'note':
					return [i18n.t('_notification.newNote', { name: getUserName(data.body.user) }), {
						body: data.body.note.text ?? '',
						icon: data.body.user.avatarUrl ?? undefined,
						badge: iconUrl('bell'),
						data,
						actions: [
							{
								action: 'reply',
								title: i18n.t('_notification._actions.reply'),
							},
							...((data.body.note.visibility === 'public' || data.body.note.visibility === 'home') ? [
								{
									action: 'renote',
									title: i18n.t('_notification._actions.renote'),
								},
							] : []),
						],
					}];
